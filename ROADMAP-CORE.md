...
This is the **core / terminal** track: interrupts, memory, drivers, shell,
filesystem, scheduling, and other kernel services that are not the graphical
UI. The GUI track lives in [`ROADMAP-GUI.md`](ROADMAP-GUI.md); the index and
track rules are in [`ROADMAP.md`](ROADMAP.md).

**How to use this file:** an automation (or human) with `TRACK=core` picks
the **first** unchecked task here (`- [ ]`), implements only that task,
verifies it, checks it off in this file, and opens a PR. Do not start GUI
tasks from this file - those belong to the GUI automation.

**Ownership (soft):** prefer touching `shell`, `fs`, `ata`, `task`,
`keyboard`, `input`, `pit`, and related kernel services. Avoid drive-by
rewrites of `framebuffer.rs` / future `gui/` modules - leave those to the
GUI track unless a core task explicitly needs a tiny shared API.

Each phase links to a skill under [`.cursor/skills/`](.cursor/skills/).
...
## Phase 6 - Shell & filesystem depth

*Skills: [`.cursor/skills/filesystem/`](.cursor/skills/filesystem/SKILL.md),
[`.cursor/skills/drivers/`](.cursor/skills/drivers/SKILL.md)*

Phases 1-4 gave GOATos a working shell and a read-only GOATFS. This phase
deepens those so a future GUI (terminal window, file browser) has real
services to call - without waiting on windowing.

- [ ] **6.1 - `ls` built-in.** List names in the mounted GOATFS directory
      from the shell (`ls` with no args).
      *Done when:* `ls` prints every file name in the image directory
      (at least `hello.txt`), verified via QEMU `sendkey` + serial and
      spot-checked in the web demo.
- [ ] **6.2 - ATA writes + GOATFS create/overwrite.** Extend the ATA PIO
      driver with sector writes and teach GOATFS enough to create or
      overwrite a small file (e.g. shell `write <file> <text>` or
      `echo ... > file` - pick one simple form).
      *Done when:* a shell command writes a file, a later `cat` of that
      file returns the exact bytes, and a reboot still shows them (image
      persistence within the running QEMU/`disk.img` session is enough;
      host-side re-pack is not required).
- [ ] **6.3 - Shell command history.** Up-arrow (or a small fixed ring)
      recalls the previous submitted line into the line editor.
      *Done when:* submit a command, press the history key, see the prior
      line restored, and can edit/submit it again.
- [ ] **6.4 - Minimal VFS surface.** Introduce a tiny
      `open`/`read`/`write`/`close`-style kernel API that the shell (and
      later the GUI) uses instead of calling GOATFS helpers directly.
      One filesystem type underneath is fine.
      *Done when:* `cat`/`ls`/the write command from 6.2 go through the
      VFS functions, and a unit-style boot self-test opens+reads
      `hello.txt` via that API.
...
Phase 4's scheduler is cooperative (`yield` only). Preemption makes
background work and a future GUI event loop share the CPU without every
task remembering to yield.

- [ ] **7.1 - Timer-driven preemption.** On a PIT tick (or every N ticks),
      request a reschedule so the running task is preempted into the
      ready queue.
      *Done when:* 3+ CPU-bound tasks that never call `yield` still
      round-robin (turn counts stay roughly fair) under CI serial checks.
- [ ] **7.2 - Sleep / timeout helper.** `sleep_ms` (or tick-based) that
      blocks the current task until a deadline without spinning the whole
      machine.
      *Done when:* a demo task sleeps ~500ms between prints and the shell
      stays responsive; serial shows the gap without wedging input.

---

## Notes (core track)

- Verify the way `.cursor/skills/qemu-testing-and-verification/` describes:
  headless serial for CI, plus a QEMU screendump when the change is visual.
- Disk I/O and boot-time BIOS paths should also be spot-checked in the
  browser demo (`.cursor/skills/web-demo-packaging/`).
- When finishing the last task of a phase, update the matching item in
  [README Status](README.md#status) if one exists.
- If a task needs a GUI-track deliverable first, mark it
  `Depends on: GUI <task id>` under the task and skip to the next
  unchecked core task that is unblocked.
